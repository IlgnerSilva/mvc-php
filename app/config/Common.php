...
include(RelativePath . "/config/db_mysqli.php");
//End Include Files

//Connection Settings @0-3D27014C
//$CCConnectionSettings = json_decode(file_get_contents(RelativePath . "/../db_credentials.json"), 1);
...
//Initialize Common Variables @0-CFBCD272
...
define("TemplatePath", RelativePath . "/templates/");
...
$CCSLocales->AddLocale("pt", Array("pt", "BR", array(1, 0, ""), 2, ",", ".", array("janeiro", "fevereiro", "março", "abril", "maio", "junho", "julho", "agosto", "setembro", "outubro", "novembro", "dezembro"), array("jan", "fev", "mar", "abr", "mai", "jun", "jul", "ago", "set", "out", "nov", "dez"), array("domingo", "segunda-feira", "terça-feira", "quarta-feira", "quinta-feira", "sexta-feira", "sábado"), array("dom", "seg", "ter", "qua", "qui", "sex", "sáb"), array("D", "S", "T", "Q", "Q", "S", "S"), array("dd", "/", "mm", "/", "yyyy"), array("dddd", ", ", "d", " de ", "mmmm", " de ", "yyyy"), array("HH", ":", "nn"), array("HH", ":", "nn", ":", "ss"), "", "", 0, false, "", "iso-8859-1", "ISO-8859-1", array(1, 7)));
$CCSLocales->AddLocale("pt-BR", Array("pt", "BR", array(1, 0, ""), 2, ".", "", array("janeiro", "fevereiro", "março", "abril", "maio", "junho", "julho", "agosto", "setembro", "outubro", "novembro", "dezembro"), array("jan", "fev", "mar", "abr", "mai", "jun", "jul", "ago", "set", "out", "nov", "dez"), array("domingo", "segunda-feira", "terça-feira", "quarta-feira", "quinta-feira", "sexta-feira", "sábado"), array("dom", "seg", "ter", "qua", "qui", "sex", "sáb"), array("D", "S", "T", "Q", "Q", "S", "S"), array("d", "/", "m", "/", "yyyy"), array("dddd", ", ", "d", " de ", "mmmm", " de ", "yyyy"), array("HH", ":", "nn"), array("HH", ":", "nn", ":", "ss"), "", "", 0, true, "", "utf-8", "UTF-8", array(1, 7)));
$CCSLocales->DefaultLocale = strtolower("pt-BR");
$CCSLocales->Init();
$Charset = "utf-8";
...
$CCSDesign = "";
define("CCS_ENCRYPTION_KEY_FOR_COOKIE", '9dP6PK55L86s870m');
...
class clsDBapp extends DB_Adapter
{
    ...
    function Initialize()
    {
        ...
        $this->SetProvider($CCConnectionSettings["app"]);
        parent::Initialize();
...
//CCSelectProjectDesign @0-E62BEB46
...
//CCGetUserID @0-1F34AEB4
function CCGetUserID()
{
    return CCGetSession("UserID_app");
}
//End CCGetUserID

//CCGetGroupID @0-19334194
function CCGetGroupID()
{
    return CCGetSession("GroupID_app");
}
//End CCGetGroupID

//CCGetUserLogin @0-34884DAC
function CCGetUserLogin()
{
    return CCGetSession("UserLogin_app");
}
//End CCGetUserLogin
...
//CCLoginUser @0-9C517795
function CCLoginUser($login, $password)
{
    CCLogoutUser();
    $db = new clsDBapp();
    $SQL = "SELECT id_usuarios, id_grupos, login_senha FROM usuarios WHERE login_usuario=" . $db->ToSQL($login, ccsText) . " AND login_senha=MD5(" . $db->ToSQL($password, ccsText) . ")";
    $db->query($SQL);
    $Result = $db->next_record();
    if ($Result) {
        CCSetSession("UserID_app", $db->f("id_usuarios"));
        CCSetSession("UserLogin_app", $login);
        CCSetSession("GroupID_app", $db->f("id_grupos"));
